...
\begin{document}
  ...
  \begin{tikzpicture}[scale=.5]
    ...
    \begin{scope}[xshift=0cm]
      ...
      \node[below] at (0,-4) {Elemento neutro $\mathcal{O}$};
    \end{scope}
    ...
    \begin{scope}[yshift=-10cm]
      ...
      \draw[name intersections={of=curve and chord}] (intersection-1) node {$●$} node[above left] {$P$}
                                                     (intersection-2) node {$●$} node[above right=-1pt] {$Q$}
                                                     (intersection-3) node {$●$} coordinate[name=mPQ];
      \draw[dashed, semithick, name path=vertical] (mPQ) ++(0,1) -- +(0,-5.5);
      \draw[name intersections={of=curve and vertical}] (intersection-2) node {$●$} node[below left] {$P+Q$};
      \node[below, align=center] at (0,-4) {Suma $P+Q$ \\ %<<Regla de >>
      };
    \end{scope}
    ...
    \begin{scope}[xshift=7.5cm, yshift=-10cm]
	  ...
	  \draw[tangent=0.265] plot[id=curve-22bis, raw gnuplot] function {
	    f(x,y) = y**2 - x**3 + 2*x - 2;
	    set xrange [-3:3];
	    set yrange [-3:3];
	    set view 0,0;
